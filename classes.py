class cls_blk:
    string crse_name
    string teach_name
    int period
def _init_(self, crse_name,teach_name):
    self.crse_name = cse_name
    self.teach_name = teach_name

def main():
    #block1=class_block instance



class student:
    string name
    int grad_yr
    def__init_(self, name, grad_year):
        self.name=name
        self.grad-yr= grad_yr
<<<<<<< HEAD
    def main():
        # usr_1 = student("shreya",2019) instance
        # usr_2 = student("melody",2020) instance
=======
        def main():
            usr_1 = student("shreya",2019)
            usr_2 = student("melody",2020)
<<<<<<< HEAD


=======
>>>>>>> c7d3f100be5a01080d3c96f12d1afb7068897690
>>>>>>> 252e546c115681803c707e85f2ef9df758501168
>>>>>>> 15e679c2af6eda9ed46044dff33ec9f41b264b44
